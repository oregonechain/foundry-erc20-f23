...
contract OurTokenTest is Test {
    ...
    // CHAT GPT Generated (Fixed to pass)


    function testTransferSimple() public {
    ...
    function testTransferFailsWithInsufficientBalance() public {
        vm.prank(makeAddr("tom"));
        vm.expectRevert();
        ourToken.transfer(address(alice), 1);
    }

    function testTransferFromFailsWithInsufficientAllowance() public {
        vm.prank(msg.sender);
        vm.expectRevert();
        ourToken.transferFrom(msg.sender, address(this), 1);
    }
